...
class ProductsMem extends ProductsDao {
  ...
  async getByCategory(category) {
    const productsDao = this.products.filter((p) => p.categoria == category);
  ...
  async updateById(id, productDto) {
    ...
    if (productDto.descripcion) {
      this.products[index].descripcion = productDto.descripcion;
    }
    if (productDto.categoria) {
      this.products[index].categoria = productDto.categoria;
    }
